...
Livonia 提供完整的 Docker 容器化支持，便于部署和测试。
...
项目包含：
- **Dockerfile** - 定义包含 OpenJDK 23 和所有必要依赖的容器镜像
- **docker-compose.yml** - 编排容器，包含用于 webapp 热部署的卷挂载
- **.dockerignore** - 通过排除不必要的文件来优化构建上下文
...
2. **Webapp 更改**：通过卷挂载自动反映
   - `webapps/` 目录中的文件更改会立即生效
   - 静态内容无需重启容器
